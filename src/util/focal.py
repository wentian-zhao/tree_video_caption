...
from util.loss import sequence_mask
...
def masked_binary_focal_loss_with_logits(
    input: torch.Tensor,
    target: torch.Tensor,
    ...
    mask = mask.to(device)
    loss_tmp = loss_tmp * mask  # (batch_size, max_len)
    loss = loss_tmp.sum() / length.float().sum().to(device)
